...
int input_shooting(struct file_content * pfc,
  ...
  char * const unknown_namestrings[] = {"h",                        /* unknown param for target '100*theta_s' */
                                        ...
                                        "anq_shooting_parameter",   /* unknown param for target 'Omega_anq' */        
                                        "sde_shooting_parameter",   /* unknown param for target 'Omega_sde' */    
                                        "a2f_shooting_parameter",   /* unknown param for target 'Omega_a2f' */     
                                        "eymh_shooting_parameter",  /* unknown param for target 'Omega_eymh'*/    
                                        "avf_shooting_parameter",   /* unknown param for target 'Omega_avf' */                                                                                                                                                                                                                                                                                
  ...
  enum computation_stage target_cs[] = {cs_thermodynamics, /* computation stage for target '100*theta_s' */
                                        ...
                                        cs_background,     /* computation stage for target 'Omega_anq' */  
                                        cs_background,     /* computation stage for target 'Omega_sde' */
                                        cs_background,     /* computation stage for target 'Omega_a2f' */    
                                        cs_background,     /* computation stage for target 'Omega_eymh' */ 
                                        cs_background,     /* computation stage for target 'Omega_avf' */                                            
...
int input_find_root(double *xzero,
  ...
  /* anq, sde a2f, eymh, avf we changed the number of attempts */  
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_scf:
      ...
      break;
    case Omega_anq:   /*Anisotropic quintaessence*/
        xguess[index_guess] = ba.anq_parameters_3;
        dxdy[index_guess] = 1.;    /* We choose that number making trial and error */
      break;
    case Omega_sde:   /*Solid dark energy*/
        xguess[index_guess] = ba.sde_parameters_3;
        dxdy[index_guess] = 1.;  /*We choose that number making trial and error */
      break;
    case Omega_a2f:   /*Anisotropic 2-form*/
        xguess[index_guess] = ba.a2f_parameters_3;
        dxdy[index_guess] = 1.;    /* We choose that number making trial and error */
      break;      
    case Omega_eymh:   /*Eintein Yang-Mills Higgs */
        xguess[index_guess] = ba.eymh_parameters_3;
        dxdy[index_guess] = 1.; /* We choose that number making trial and error */
      break;  
    case Omega_avf:   /*Anisotropic vector field*/
        xguess[index_guess] = ba.avf_parameters_2;
        dxdy[index_guess] = 1.;    /* We choose that number making trial and error */
      break;                                  
    case omega_ini_dcdm:
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_scf:
      ...
      break;
    case Omega_anq:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_anq]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]-ba.Omega0_lambda;
      break;         
    case Omega_sde:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_sde]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]-ba.Omega0_lambda;
      break; 
    case Omega_a2f:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_a2f]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]-ba.Omega0_lambda;      
      break; 
    case Omega_eymh:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_eymh]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]-ba.Omega0_lambda;
      break;   
    case Omega_avf:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_avf]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]/ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_H]-ba.Omega0_lambda;
      break;                            
    case Omega_ini_dcdm:
    case omega_ini_dcdm:
      rho_dcdm_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** - Define local variables */
  int flag1, flag2, flag3, flag4, flag5, flag6, flag7, flag8;  /** anq, sde, a2f, eymh, avf  new flag*/
  double param1, param2, param3, param4, param5, param6, param7, param8; /** anq, sde, a2f, eymh, avf  new param*/
  char string1[_ARGUMENT_LENGTH_MAX_];
  int fileentries;
  ...
  class_call(parser_read_double(pfc,"Omega_scf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_anq",&param4,&flag4,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_sde",&param5,&flag5,errmsg),
             errmsg,
             errmsg);                 
  class_call(parser_read_double(pfc,"Omega_a2f",&param6,&flag6,errmsg),
             errmsg,
             errmsg);    
  class_call(parser_read_double(pfc,"Omega_eymh",&param7,&flag7,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_avf",&param8,&flag8,errmsg),
             errmsg,
             errmsg);         
  /* Test */
  ...
  class_test((flag4 == _TRUE_ && (param4 != 0)) && ((flag1 == _TRUE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_) || (param5 != 0) || (param6 != 0) || (param7 != 0) || (param8 != 0)),
             errmsg,
             "You have entered 'Omega_anq' > 0 , so you have to left unspecified 'Omega_lambda' and  specify 'Omega_fld', 'Omega_scf','Omega_sde','Omega_a2f', 'Omega_eymh' and 'Omega_avf' to zero'.");     
  class_test((flag5 == _TRUE_ && (param5!= 0)) && ((flag1 == _TRUE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_) || (param4 != 0) || (param6 != 0) || (param7 != 0) || (param8 != 0)),
             errmsg,
             "You have entered 'Omega_sde' > 0 , so you have to left unspecified 'Omega_lambda' and  specify 'Omega_fld', 'Omega_scf','Omega_anq','Omega_a2f' , 'Omega_eymh' and 'Omega_avf' to zero'.");     
  class_test((flag6 == _TRUE_ && (param6 != 0)) && ((flag1 == _TRUE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_) || (param4 != 0) || (param5 != 0)  || (param7 != 0) || (param8 != 0)),
             errmsg,
             "You have entered 'Omega_a2f' > 0 , so you have to left unspecified 'Omega_lambda' and  specify 'Omega_fld', 'Omega_scf', 'Omega_anq','Omega_sde' , 'Omega_eymh' and 'Omega_avf' to zero'.");     
  class_test((flag7 == _TRUE_ && (param7 != 0)) && ((flag1 == _TRUE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_) || (param4 != 0) || (param5 != 0)  || (param6 != 0) || (param8 != 0)),
             errmsg,
             "You have entered 'Omega_eymh' > 0 , so you have to left unspecified 'Omega_lambda' and  specify 'Omega_fld', 'Omega_scf', 'Omega_anq','Omega_sde' , 'Omega_a2f' and 'Omega_avf' to zero'.");     
  class_test((flag8 == _TRUE_ && (param8 != 0)) && ((flag1 == _TRUE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_) || (param4 != 0) || (param5 != 0)  || (param6 != 0) || (param7 != 0)),
             errmsg,
             "You have entered 'Omega_eymh' > 0 , so you have to left unspecified 'Omega_lambda' and  specify 'Omega_fld', 'Omega_scf', 'Omega_anq','Omega_sde', 'Omega_a2f' and 'Omega_eymh' to zero'.");       
  /* Complete set of parameters
  ...
  //anq
  if (flag4 == _TRUE_){
    pba->Omega0_anq = param4;
  }  
  //sde
  if (flag5 == _TRUE_){
    pba->Omega0_sde = param5;
  }  
  //a2f
  if (flag6 == _TRUE_){
    pba->Omega0_a2f = param6;
  }  
  //EYMH
  if (flag7 == _TRUE_){
    pba->Omega0_eymh = param7;
  }    
  //avf
  if (flag8 == _TRUE_){
    pba->Omega0_avf = param8;
  }         
  /* Step 2 */
  if (flag1 == _FALSE_) {
    /* Fill with Lambda */
  ...
  //anq
  else if (flag4 == _FALSE_){
    pba->Omega0_anq = 0.;
  }
// sde
  else if (flag5 == _FALSE_) {
    pba->Omega0_sde = 0; 
    }
// a2f  
  else if (flag6 == _FALSE_) {
    pba->Omega0_a2f = 0;
    }
// EYMH  
  else if (flag7 == _FALSE_) {
    pba->Omega0_eymh = 0;
    } 
  //avf
  else if (flag8 == _FALSE_){
    pba->Omega0_avf = 0.;
  }             
  /*   
 ...
 /** 8.c) If Omega anisotropic quintaessence is different from 0  (anq) */
  if (pba->Omega0_anq != 0.){

    class_read_double("anq_parameters_1",pba->anq_parameters_1);
    class_read_double("anq_parameters_2",pba->anq_parameters_2);
    class_read_double("anq_parameters_3",pba->anq_parameters_3);

    /** 8.c.2) Shooting parameter */
    /* Read */
    class_read_double("anq_shooting_parameter",pba->anq_parameters_3);
  }


 /** 8.d) If Omega solid dark energy is different from 0  (sde)*/

  if (pba->Omega0_sde != 0.){

    class_read_double("sde_parameters_1",pba->sde_parameters_1);
    class_read_double("sde_parameters_2",pba->sde_parameters_2);
    class_read_double("sde_parameters_3",pba->sde_parameters_3);

    /** 8.d.2) Shooting parameter */
    /* Read */
     class_read_double("sde_shooting_parameter",pba->sde_parameters_3); 
  }


 /** 8.e) If Omega anisotropic 2-form is different from 0 (a2f)*/

  if (pba->Omega0_a2f != 0.){

    class_read_double("a2f_parameters_1",pba->a2f_parameters_1);
    class_read_double("a2f_parameters_2",pba->a2f_parameters_2);
    class_read_double("a2f_parameters_3",pba->a2f_parameters_3);
    class_read_double("a2f_parameters_4",pba->a2f_parameters_4);

    /** 8.e.2) Shooting parameter */
    /* Read */
    class_read_double("a2f_shooting_parameter",pba->a2f_parameters_3);
  }  

 /** 8.f) If Omega Einstein Yang-Mill Higss is different from 0  (EYMH) */
  if (pba->Omega0_eymh != 0.){

    class_read_double("eymh_parameters_1",pba->eymh_parameters_1);
    class_read_double("eymh_parameters_2",pba->eymh_parameters_2);
    class_read_double("eymh_parameters_3",pba->eymh_parameters_3); //waqui

    /** 8.f.2) Shooting parameter */
    /* Read */
    class_read_double("eymh_shooting_parameter",pba->eymh_parameters_3); //aqui

  }

 /** 8.g) If Omega anisotropic vector field is different from 0  (avf) */
  if (pba->Omega0_avf != 0.){

    class_read_double("avf_parameters_1",pba->avf_parameters_1);
    class_read_double("avf_parameters_2",pba->avf_parameters_2);

    /** 8.g.2) Shooting parameter */
    /* Read */
    class_read_double("avf_shooting_parameter",pba->avf_parameters_2);
  }  
...
int input_default_params(struct background *pba,
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_anq = 0.;  //anq    
  pba->Omega0_sde = 0.;  //sde 
  pba->Omega0_a2f = 0.;  //a2f  
  pba->Omega0_eymh = 0.; //EYMH
  pba->Omega0_avf = 0.;  //avf     
  ...
  /** anq anisotropic quintaessence parameters and initial condition */
  pba->anq_parameters_1 = 0.1;
  pba->anq_parameters_2 = 1.e3;
  pba->anq_parameters_3 = 0.87;
  /** sde Solid dark energy parameters and initial condition */
  pba->sde_parameters_1 = 0.01;
  pba->sde_parameters_2 = 0.001;
  pba->sde_parameters_3 = 5.5; 
  /** a2f anisotropic 2-form*/
  pba->a2f_parameters_1 = 2.;
  pba->a2f_parameters_2 = 30.;
  pba->a2f_parameters_3 = 6.;
  pba->a2f_parameters_4 = pow(10.,-22);
  /** EYMH Einstein Yang-Mills Higss */
  pba->eymh_parameters_1 = 1;
  pba->eymh_parameters_2 = pow(10.,-17);
  pba->eymh_parameters_3 = 8.9;         
  /** avf anisotropic vector field parameters and initial condition */
  pba->avf_parameters_1 = 0.1;  
  pba->avf_parameters_2 = 1.;
  /**
