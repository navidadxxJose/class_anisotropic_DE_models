...
int background_functions(
  ...
  int n_ncdm;
   /* Hubble's parameter*/   
  double H;   //anq, sde, a2f, eymh, avf  
  /* fluid's time-dependent equation of state parameter */
  double w_fld, dw_over_da, integral_fld;
  /* scalar field quantities */
  double phi, phi_prime;
  /* anisotropic quintessence quantities   anq */  
  double rho_anq, p_anq, w_anq, x_anq, y_anq, z_anq, s_anq;
  /* anisotropic quintessence parameters    anq */
  double l_anq, mu_anq;  // \lambda \mu    
  /* Solid dark energy quantities  sde */  
  double rho_sde, p_sde, w_sde, f1_sde, f2_sde, s_sde;
  /* Solid dark energy parameters    sde */
  double n_sde, m_sde;  // n, m      
  /* anisotropic 2-form quantities */  
  double rho_a2f, p_a2f, w_a2f, x1_a2f, x2_a2f, OmB_a2f, s_a2f;
  /* 2-form parameters    a2f */
  double l_a2f, m_a2f;  // \lambda, \mu  
  /* Einstein Yang-Mills Higgs   eymh */  
  double rho_eymh, p_eymh, w_eymh, x_eymh, y_eymh, xi_eymh, pp_eymh, sp_eymh, z_eymh, wp_eymh, v_eymh, s_eymh;
  /* Einstein Yang-Mills Higgs parameters  eymh */
  double ap_eymh;  // \alpha         
  /* anisotropic vector field quantities   avf */  
  double rho_avf, p_avf, z_avf, v_avf, x_avf, s_avf;
  /* anisotropic vector field parameters    avf */
  double l_avf;  // \lambda       
  ...
  /* Anisotropic quintaessence anq */
  if (pba->has_anq == _TRUE_){

   /* Quintaessence parameters*/
    l_anq  = pba->anq_parameters_1;             // \lambda
    mu_anq = pba->anq_parameters_2;             // \mu

  /* two-form fields */
    x_anq = pvecback_B[pba->index_bi_x_anq]; //Short notation  
    y_anq = pvecback_B[pba->index_bi_y_anq];     
    z_anq = pvecback_B[pba->index_bi_z_anq];     
    s_anq = pvecback_B[pba->index_bi_s_anq];                   
    pvecback[pba->index_bg_x_anq] = x_anq; 
    pvecback[pba->index_bg_y_anq] = y_anq;
    pvecback[pba->index_bg_z_anq] = z_anq;
    pvecback[pba->index_bg_s_anq] = s_anq;

  /* density energy of the quintaessence field.  */
    pvecback[pba->index_bg_rho_anq] = x_anq*x_anq + y_anq*y_anq + z_anq*z_anq + s_anq*s_anq; // density energy of the quintaessence field.  
    rho_tot += pvecback[pba->index_bg_rho_anq]; 
  } 

/* Solid dark energy  sde */
  if (pba->has_sde == _TRUE_){

   /* Solid parameters*/
    n_sde  = pba->sde_parameters_1;         //e^{n}
    m_sde  = pba->sde_parameters_2;         //e^{m}

  /* Solid fields */
    f1_sde = pvecback_B[pba->index_bi_f1_sde]; //Short notation
    f2_sde = pvecback_B[pba->index_bi_f2_sde];     
    s_sde  = pvecback_B[pba->index_bi_s_sde];                 
    pvecback[pba->index_bg_f1_sde] = f1_sde;
    pvecback[pba->index_bg_f2_sde] = f2_sde;
    pvecback[pba->index_bg_s_sde]  = s_sde;

  /* density energy of the solid */ 
    pvecback[pba->index_bg_rho_sde] = f1_sde*f1_sde + 2.*f2_sde*f2_sde + s_sde*s_sde; 
    rho_tot += pvecback[pba->index_bg_rho_sde]; 
  } 

/* Anisotropic 2 form*/
  if (pba->has_a2f == _TRUE_){

   /* Vector field parameters*/

    l_a2f  = pba->a2f_parameters_1;             // \lambda
    m_a2f  = pba->a2f_parameters_2;             // \mu

  /* anisotropic two-form fields */
    x1_a2f = pvecback_B[pba->index_bi_x1_a2f]; //Short notation 
    x2_a2f = pvecback_B[pba->index_bi_x2_a2f]; 
    s_a2f = pvecback_B[pba->index_bi_s_a2f];             
    OmB_a2f = pvecback_B[pba->index_bi_OmB_a2f];    
    pvecback[pba->index_bg_x1_a2f] = x1_a2f; 
    pvecback[pba->index_bg_x2_a2f] = x2_a2f;
    pvecback[pba->index_bg_s_a2f] = s_a2f;
    pvecback[pba->index_bg_OmB_a2f] = OmB_a2f;

  /* density energy of the vector field */  
    pvecback[pba->index_bg_rho_a2f] = OmB_a2f + x1_a2f*x1_a2f + x2_a2f*x2_a2f + s_a2f*s_a2f;     
    rho_tot += pvecback[pba->index_bg_rho_a2f]; 
  }

/* Einstein Yang-Mills Higgs */
  if (pba->has_eymh == _TRUE_){

   /* Einstein Yang-Mills Higgs parameters */
    ap_eymh  = pba->eymh_parameters_1;             // \alpha

  /* Einstein Yang-Mills Higgs filds */
    x_eymh  = pvecback_B[pba->index_bi_x_eymh]; //Short notation  
    y_eymh  = pvecback_B[pba->index_bi_y_eymh]; 
    xi_eymh  = pvecback_B[pba->index_bi_xi_eymh]; 
    pp_eymh = pvecback_B[pba->index_bi_pp_eymh]; 
    sp_eymh = pvecback_B[pba->index_bi_sp_eymh];
    z_eymh  = pvecback_B[pba->index_bi_z_eymh];
    wp_eymh = pvecback_B[pba->index_bi_wp_eymh];      
    v_eymh  = pvecback_B[pba->index_bi_v_eymh];  
    s_eymh  = pvecback_B[pba->index_bi_s_eymh];             
      
    pvecback[pba->index_bg_x_eymh]  = x_eymh; 
    pvecback[pba->index_bg_y_eymh]  = y_eymh;
    pvecback[pba->index_bg_xi_eymh] = xi_eymh;
    pvecback[pba->index_bg_pp_eymh] = pp_eymh;
    pvecback[pba->index_bg_sp_eymh] = sp_eymh;
    pvecback[pba->index_bg_z_eymh]  = z_eymh;
    pvecback[pba->index_bg_wp_eymh] = wp_eymh;
    pvecback[pba->index_bg_v_eymh]  = v_eymh;
    pvecback[pba->index_bg_s_eymh]  = s_eymh;

  /* density energy of the Einstein Yang-Mills Higgs */  
    pvecback[pba->index_bg_rho_eymh] = 0.5*x_eymh*x_eymh + y_eymh*y_eymh + pp_eymh*pp_eymh*sp_eymh*sp_eymh + 0.5*sp_eymh*sp_eymh*sp_eymh*sp_eymh + z_eymh*z_eymh +   wp_eymh*wp_eymh + v_eymh*v_eymh + s_eymh*s_eymh;   
    rho_tot += pvecback[pba->index_bg_rho_eymh]; 
  }   

  /* Anisotropic vector field avf */
  if (pba->has_avf == _TRUE_){

   /* Vector field parameter*/
    l_avf  = pba->avf_parameters_1;             // \lambda

  /* vector fields dynamic variables */
    z_avf = pvecback_B[pba->index_bi_z_avf]; //Short notation  
    v_avf = pvecback_B[pba->index_bi_v_avf];     
    x_avf = pvecback_B[pba->index_bi_x_avf];     
    s_avf = pvecback_B[pba->index_bi_s_avf];                   
    pvecback[pba->index_bg_z_avf] = z_avf; 
    pvecback[pba->index_bg_v_avf] = v_avf;
    pvecback[pba->index_bg_x_avf] = x_avf;
    pvecback[pba->index_bg_s_avf] = s_avf;

  /* density energy of the quintaessence field.  */
    pvecback[pba->index_bg_rho_avf] = z_avf*z_avf + v_avf*v_avf + s_avf*s_avf; // density energy of the vector field.  
    rho_tot += pvecback[pba->index_bg_rho_avf]; 
  } 
  ...
  H = sqrt(rho_tot-pba->K/a/a); /*  Short notations for use in the following equations*/ //sde, anq, a2f, eymh, avf
  pvecback[pba->index_bg_H] = sqrt(rho_tot-pba->K/a/a);

/* Pressure of the quintaessence  anq */
  if (pba->has_anq == _TRUE_){

    w_anq = -1. + 2.*(3.*x_anq*x_anq + 2.*z_anq*z_anq + 3.*s_anq*s_anq)/3./(x_anq*x_anq + y_anq*y_anq + z_anq*z_anq + s_anq*s_anq); 

   /* Dark energy pressure. P_anq = w_anq*\rho_anq */
    pvecback[pba->index_bg_p_anq] = w_anq*pvecback[pba->index_bg_rho_anq];
    p_tot += pvecback[pba->index_bg_p_anq];
    dp_dloga += 0.0; 
  }   

/* Pressure of the Solid sde */
  if (pba->has_sde == _TRUE_){

    w_sde = -1. + 2.*(n_sde*f1_sde*f1_sde +2.*m_sde*f2_sde*f2_sde + 3.*s_sde*s_sde)/3./(f1_sde*f1_sde +2.*f2_sde*f2_sde + s_sde*s_sde);

   /* Dark energy pressure. P_sde = w_sde*\rho_sde */
    pvecback[pba->index_bg_p_sde] = w_sde*pvecback[pba->index_bg_rho_sde];
    p_tot += pvecback[pba->index_bg_p_sde];
    dp_dloga += 0.0; 
  } 

/*  Pressure of the dark energy in the anisotropic 2-form model */
  if (pba->has_a2f == _TRUE_){

    w_a2f = (3*(x1_a2f*x1_a2f - x2_a2f*x2_a2f + s_a2f*s_a2f) - OmB_a2f)/(3*(x1_a2f*x1_a2f + x2_a2f*x2_a2f + s_a2f*s_a2f + OmB_a2f));

   /* Dark energy pressure. P_a2f = w_a2f*\rho_a2f */
    pvecback[pba->index_bg_p_a2f] = w_a2f*pvecback[pba->index_bg_rho_a2f];
    p_tot += pvecback[pba->index_bg_p_a2f];
    dp_dloga += 0.0; 
  } 

/*  Pressure of the Einstein Ynag-Mills Higgs field */
  if (pba->has_eymh == _TRUE_){

    w_eymh = (sp_eymh*sp_eymh*(pp_eymh*pp_eymh + 0.5*sp_eymh*sp_eymh) - 3.*v_eymh*v_eymh - wp_eymh*wp_eymh + 0.5*x_eymh*x_eymh + y_eymh*y_eymh + 3.*z_eymh*z_eymh + 3.*s_eymh*s_eymh)/(3.*(sp_eymh*sp_eymh*(pp_eymh*pp_eymh +0.5*sp_eymh*sp_eymh) + v_eymh*v_eymh + wp_eymh*wp_eymh + 0.5*x_eymh*x_eymh + y_eymh*y_eymh + z_eymh*z_eymh + s_eymh*s_eymh)); 

   /* Dark energy pressure. P_eymh = w_eymh \rho_eymh */
    pvecback[pba->index_bg_p_eymh] = w_eymh*pvecback[pba->index_bg_rho_eymh];
    p_tot += pvecback[pba->index_bg_p_eymh];
    dp_dloga += 0.0; 
  } 

/* Pressure of the vector field  avf */
  if (pba->has_avf == _TRUE_){

    pvecback[pba->index_bg_p_avf] = (z_avf*z_avf - v_avf*v_avf*(3. + l_avf*x_avf*x_avf) + 3.*s_avf)/3.;
    p_tot += pvecback[pba->index_bg_p_avf];
    dp_dloga += 0.0; 
  }   
...
int background_indices(
  ...
  pba->has_dr = _FALSE_;
  pba->has_scf = _FALSE_;
  pba->has_anq = _FALSE_;   //anq     
  pba->has_sde = _FALSE_;   //sde   
  pba->has_a2f = _FALSE_;   //a2f   
  pba->has_eymh = _FALSE_;  //EYMH    
  pba->has_avf = _FALSE_;   //avf   
  ...
  if (pba->Omega0_anq != 0. && pba->Omega0_lambda != 0.)  //anq
    pba->has_anq = _TRUE_;   

  if (pba->Omega0_sde != 0. && pba->Omega0_lambda !=0.)  //sde
    pba->has_sde = _TRUE_;    

  if (pba->Omega0_a2f != 0. && pba->Omega0_lambda !=0.)  //a2f
    pba->has_a2f = _TRUE_;   

  if (pba->Omega0_eymh != 0. && pba->Omega0_lambda !=0.)  //eymh
    pba->has_eymh = _TRUE_;    

  if (pba->Omega0_avf != 0. && pba->Omega0_lambda != 0.)  //avf
    pba->has_avf = _TRUE_;   

  if (pba->Omega0_lambda != 0. && pba->Omega0_anq == 0.  && pba->Omega0_sde == 0. && pba->Omega0_a2f == 0.  && pba->Omega0_eymh == 0. && pba->Omega0_avf == 0.)
  ...
  class_define_index(pba->index_bg_p_prime_scf,pba->has_scf,index_bg,1);
    ...
    /* - index for anisotropic quintessence   anq */
  class_define_index(pba->index_bg_y_anq,pba->has_anq,index_bg,1);
  class_define_index(pba->index_bg_z_anq,pba->has_anq,index_bg,1);
  class_define_index(pba->index_bg_s_anq,pba->has_anq,index_bg,1);  
  class_define_index(pba->index_bg_rho_anq,pba->has_anq,index_bg,1);
  class_define_index(pba->index_bg_p_anq,pba->has_anq,index_bg,1);
  class_define_index(pba->index_bg_x_anq,pba->has_anq,index_bg,1);    

  /* - index for the Solid  sde */
  class_define_index(pba->index_bg_f1_sde,pba->has_sde,index_bg,1);
  class_define_index(pba->index_bg_f2_sde,pba->has_sde,index_bg,1);
  class_define_index(pba->index_bg_s_sde,pba->has_sde,index_bg,1);  
  class_define_index(pba->index_bg_rho_sde,pba->has_sde,index_bg,1);
  class_define_index(pba->index_bg_p_sde,pba->has_sde,index_bg,1);

   /* - index for anisotropic 2-form a2f */
  class_define_index(pba->index_bg_x1_a2f,pba->has_a2f,index_bg,1);
  class_define_index(pba->index_bg_x2_a2f,pba->has_a2f,index_bg,1);
  class_define_index(pba->index_bg_s_a2f,pba->has_a2f,index_bg,1);
  class_define_index(pba->index_bg_OmB_a2f,pba->has_a2f,index_bg,1);  
  class_define_index(pba->index_bg_rho_a2f,pba->has_a2f,index_bg,1);
  class_define_index(pba->index_bg_p_a2f,pba->has_a2f,index_bg,1); 

    /* - index for Einstein Yang Mill-Higgs EYMH */
  class_define_index(pba->index_bg_x_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_y_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_xi_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_pp_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_sp_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_z_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_wp_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_v_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_s_eymh,pba->has_eymh,index_bg,1);  
  class_define_index(pba->index_bg_rho_eymh,pba->has_eymh,index_bg,1);
  class_define_index(pba->index_bg_p_eymh,pba->has_eymh,index_bg,1);

    /* - index for anisotropic vector field   avf */
  class_define_index(pba->index_bg_z_avf,pba->has_avf,index_bg,1);
  class_define_index(pba->index_bg_v_avf,pba->has_avf,index_bg,1);
  class_define_index(pba->index_bg_x_avf,pba->has_avf,index_bg,1);  
  class_define_index(pba->index_bg_s_avf,pba->has_avf,index_bg,1);    
  class_define_index(pba->index_bg_rho_avf,pba->has_avf,index_bg,1);
  class_define_index(pba->index_bg_p_avf,pba->has_avf,index_bg,1);
  ...
  class_define_index(pba->index_bi_phi_prime_scf,pba->has_scf,index_bi,1);
    ...
    /* -> Quintaessence wrt conformal time (Zuma)  anq */
  class_define_index(pba->index_bi_x_anq,pba->has_anq,index_bi,1);
  class_define_index(pba->index_bi_y_anq,pba->has_anq,index_bi,1);
  class_define_index(pba->index_bi_z_anq,pba->has_anq,index_bi,1);  
  class_define_index(pba->index_bi_s_anq,pba->has_anq,index_bi,1);

  /* ->Solid wrt conformal time (Zuma)  sde */
  class_define_index(pba->index_bi_f1_sde,pba->has_sde,index_bi,1);
  class_define_index(pba->index_bi_f2_sde,pba->has_sde,index_bi,1);
  class_define_index(pba->index_bi_s_sde,pba->has_sde,index_bi,1);

  /* -> Abisotropic 2-form wrt conformal time (Zuma) a2f */
  class_define_index(pba->index_bi_x1_a2f,pba->has_a2f,index_bi,1);
  class_define_index(pba->index_bi_x2_a2f,pba->has_a2f,index_bi,1);
  class_define_index(pba->index_bi_s_a2f,pba->has_a2f,index_bi,1);
  class_define_index(pba->index_bi_OmB_a2f,pba->has_a2f,index_bi,1);

  /* -> Einstein Yang-Mill Higgs wrt conformal time (Zuma) EYMH */
  class_define_index(pba->index_bi_x_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_y_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_xi_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_pp_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_sp_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_z_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_wp_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_v_eymh,pba->has_eymh,index_bi,1);
  class_define_index(pba->index_bi_s_eymh,pba->has_eymh,index_bi,1);       

    /* -> Quintaessence wrt conformal time (Zuma)  avf */
  class_define_index(pba->index_bi_z_avf,pba->has_avf,index_bi,1);
  class_define_index(pba->index_bi_v_avf,pba->has_avf,index_bi,1);
  class_define_index(pba->index_bi_x_avf,pba->has_avf,index_bi,1);  
  class_define_index(pba->index_bi_s_avf,pba->has_avf,index_bi,1);
...
int background_solve(
  ...
  int index_loga, index_scf, index_anq, index_sde, index_a2f, index_eymh, index_avf;
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_anq == _TRUE_) {
      printf("    anisotropic quintessence (anq) details:\n");
      printf("     -> Omega_Lambda = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_anq]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> H0 = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H], pba->H0);
      printf("lambda -> %.8e \n",pba->anq_parameters_1);
      printf("mu -> %.8e \n",pba->anq_parameters_2); 
      printf("anq - initial conditions -> y_ini ->  %.8e \n",pba->anq_parameters_3*1.e-26);
    }  
    if (pba->has_sde == _TRUE_) {
      printf("    Solid Dark energy (sde) details:\n");
      printf("     -> Omega_sde = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_sde]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> H0 = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H], pba->H0);
      printf("n -> %.8e \n",pba->sde_parameters_1);
      printf("m -> %.8e \n",pba->sde_parameters_2); 
      printf("sde-initial conditions -> f_ini -> %.8e \n",pba->sde_parameters_3*pow(10.,-27));
    } 
    if (pba->has_a2f == _TRUE_) {
      printf("    Anisotropic 2-form details (a2f):\n");
      printf("     -> Omega_a2f = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_a2f]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> H0 = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H], pba->H0);
      printf("lambda -> %.8e \n",pba->a2f_parameters_1);
      printf("mu -> %.8e \n",pba->a2f_parameters_2);
      printf("a2f - initial conditions -> x2_ini ->%.8e \n",pba->a2f_parameters_3*pow(10.,-27));
      printf("anq - initial conditions -> Omeha_{B,ini} ->  %.8e \n",exp(pba->a2f_parameters_4));
    }  
     if (pba->has_eymh == _TRUE_) {
      printf("  Einstein Yang-Mills Higgs details (eymh):\n");
      printf("     -> Omega_eymh = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_eymh]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> H0 = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H], pba->H0);
      printf("alpha -> %.8e \n",pba->eymh_parameters_1);
      printf("w_ini -> %.8e \n",exp(pba->eymh_parameters_2));
      printf("eymh - initial conditions -> v_{ini} %.8e \n", pba->eymh_parameters_3*pow(10.,-27));
    }
    if (pba->has_avf == _TRUE_) {
      printf("    anisotropic vector field (avf) details:\n");
      printf("     -> Omega_Lambda = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_avf]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_crit], pba->Omega0_lambda);
      printf("     -> H0 = %.8e, wished %.8e\n",
             pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H], pba->H0);
      printf("lambda -> %.8e \n",pba->avf_parameters_1);
      printf("avf - initial conditions -> v_ini ->  %.8e \n",pba->avf_parameters_2*1.e-26);
    }      
  }
  ...
  // Shear 
  if (pba->has_anq == _TRUE_) {
    pba->sigma0_shear = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_s_anq]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H];
    pba->log_sigma0_shear = log(fabs(pba->sigma0_shear)); 
  }   
  if (pba->has_sde == _TRUE_) {
    pba->sigma0_shear = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_s_sde]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H];
    pba->log_sigma0_shear = log(fabs(pba->sigma0_shear)); 
  }  
  if (pba->has_a2f == _TRUE_) {
    pba->sigma0_shear = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_s_a2f]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H];
    pba->log_sigma0_shear = log(fabs(pba->sigma0_shear)); 
  }
  if (pba->has_eymh == _TRUE_) {
    pba->sigma0_shear = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_s_eymh]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H];
    pba->log_sigma0_shear = log(fabs(pba->sigma0_shear)); 
  }   
  if (pba->has_avf == _TRUE_) {
    pba->sigma0_shear = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_s_avf]/pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H];
    pba->log_sigma0_shear = log(fabs(pba->sigma0_shear)); 
  }  
...
int background_initial_conditions(
  ...
  double H_ini;  //anq, sde, a2f, avf
  double Xi_anq, Yi_anq, Zi_anq;   // anq    
  double F1i_sde;   //sde  
  double X1i_a2f, X2i_a2f, OmBi_a2f; //a2f    
  double Xini_eymh, Yi_eymh, xiini_eymh, ppi_eymh, spi_eymh, Zi_eymh, Wpi_eymh, Vi_eymh, Si_eymh; //eymh        
  double Zi_avf, Vi_avf, Xi_avf, Si_avf;   // avf 
  ...
  */
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_scf]) ||
     ...
     // - Fix initial value for anistropic quintessence $

 if (pba->has_anq == _TRUE_) {      

   Xi_anq = 1.e-15;
   Yi_anq = pba->anq_parameters_3*1.e-26;
   Zi_anq = 1.e-12;

   H_ini = pba->H0*sqrt((pba->Omega0_g/pow(a,4.) + (pba->Omega0_b + pba->Omega0_cdm)/pow(a,3.))/(1. - Xi_anq*Xi_anq - Yi_anq*Yi_anq - Zi_anq*Zi_anq)); 

   pvecback_integration[pba->index_bi_x_anq] = Xi_anq*H_ini;
   pvecback_integration[pba->index_bi_y_anq] = Yi_anq*H_ini; 
   pvecback_integration[pba->index_bi_z_anq] = Zi_anq*H_ini; 
   pvecback_integration[pba->index_bi_s_anq] = 0.;       


    class_test(!isfinite(pvecback_integration[pba->index_bi_x_anq]) ||
               !isfinite(pvecback_integration[pba->index_bi_x_anq]),
               pba->error_message,
               "initial x_anq_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_x_anq]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_y_anq]) ||
               !isfinite(pvecback_integration[pba->index_bi_y_anq]),
               pba->error_message,
               "initial y_anq_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_y_anq]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_z_anq]) ||
               !isfinite(pvecback_integration[pba->index_bi_z_anq]),
               pba->error_message,
               "initial z_anq_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_z_anq]);     

    class_test(!isfinite(pvecback_integration[pba->index_bi_s_anq]) ||
               !isfinite(pvecback_integration[pba->index_bi_s_anq]),
               pba->error_message,
               "initial s_anq_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_s_anq]);                      
  }

     // - Fix initial value for solid sde $
 if (pba->has_sde == _TRUE_) {      

   F1i_sde = pba->sde_parameters_3*pow(10.,-27);

   H_ini = pba->H0*sqrt((pba->Omega0_g/pow(a,4) + (pba->Omega0_b + pba->Omega0_cdm)/pow(a,3))/(1.-2.*F1i_sde*F1i_sde)); 

   pvecback_integration[pba->index_bi_f1_sde] = F1i_sde*H_ini; 
   pvecback_integration[pba->index_bi_f2_sde] = F1i_sde*H_ini; 
   pvecback_integration[pba->index_bi_s_sde] = 0.;       

    class_test(!isfinite(pvecback_integration[pba->index_bi_f1_sde]) ||
               !isfinite(pvecback_integration[pba->index_bi_f1_sde]),
               pba->error_message,
               "initial f1_sde_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_f1_sde]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_f2_sde]) ||
               !isfinite(pvecback_integration[pba->index_bi_f2_sde]),
               pba->error_message,
               "initial f2_sde_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_f2_sde]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_s_sde]) ||
               !isfinite(pvecback_integration[pba->index_bi_s_sde]),
               pba->error_message,
               "initial s_sde_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_s_sde]);                    
  }

   // - Fix initial value  for anisotropy 2-form $

 if (pba->has_a2f == _TRUE_) {      
   X1i_a2f  = pow(10.,-20);
   X2i_a2f  = pba->a2f_parameters_3*pow(10.,-27);
   OmBi_a2f = pba->a2f_parameters_4;

   H_ini = pba->H0*sqrt((pba->Omega0_g/pow(a,4) + (pba->Omega0_b + pba->Omega0_cdm)/pow(a,3))/(1 - X1i_a2f*X1i_a2f - X2i_a2f*X2i_a2f - OmBi_a2f)); 

   pvecback_integration[pba->index_bi_x1_a2f] = X1i_a2f*H_ini; 
   pvecback_integration[pba->index_bi_x2_a2f] = X2i_a2f*H_ini;
   pvecback_integration[pba->index_bi_s_a2f] =  0.;       
   pvecback_integration[pba->index_bi_OmB_a2f] = OmBi_a2f*H_ini*H_ini;
   
    class_test(!isfinite(pvecback_integration[pba->index_bi_x1_a2f]) ||
               !isfinite(pvecback_integration[pba->index_bi_x1_a2f]),
               pba->error_message,
               "initial x1_a2f_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_x1_a2f]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_x2_a2f]) ||
               !isfinite(pvecback_integration[pba->index_bi_x2_a2f]),
               pba->error_message,
               "initial x2_a2f_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_x2_a2f]);     
              
    class_test(!isfinite(pvecback_integration[pba->index_bi_OmB_a2f]) ||
               !isfinite(pvecback_integration[pba->index_bi_OmB_a2f]),
               pba->error_message,
               "initial OmB_a2f_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_OmB_a2f]);                         
              
    class_test(!isfinite(pvecback_integration[pba->index_bi_s_a2f]) ||
               !isfinite(pvecback_integration[pba->index_bi_s_a2f]),
               pba->error_message,
               "initial s_a2f_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_s_a2f]);                    
  }    


     // - Fix initial value  for the Einstein Yang-Mills Higgs fields $
 if (pba->has_eymh == _TRUE_) {      
   Xini_eymh   = pow(10.,-13);
   Yi_eymh     = Xini_eymh;
   xiini_eymh  = Xini_eymh;
   Wpi_eymh    = exp(pba->eymh_parameters_2); 
   Zi_eymh     = pow(10.,-17); 
   ppi_eymh    = pow(10.,-20);
   spi_eymh    = ppi_eymh;
   Vi_eymh     = pba->eymh_parameters_3*pow(10.,-27);
   Si_eymh     = pow(10.,-20);

   H_ini = pba->H0*sqrt((pba->Omega0_g/pow(a,4) + (pba->Omega0_b + pba->Omega0_cdm)/pow(a,3))/(1-0.5*Xini_eymh*Xini_eymh - Yi_eymh*Yi_eymh - ppi_eymh*ppi_eymh*spi_eymh*spi_eymh - 0.5*spi_eymh*spi_eymh*spi_eymh*spi_eymh - Zi_eymh*Zi_eymh - Wpi_eymh*Wpi_eymh - Vi_eymh*Vi_eymh - Si_eymh*Si_eymh));

   pvecback_integration[pba->index_bi_x_eymh]  = Xini_eymh*H_ini; 
   pvecback_integration[pba->index_bi_y_eymh]  = Yi_eymh*H_ini;
   pvecback_integration[pba->index_bi_xi_eymh] = xiini_eymh; 
   pvecback_integration[pba->index_bi_wp_eymh] = Wpi_eymh*H_ini;
   pvecback_integration[pba->index_bi_z_eymh]  = Zi_eymh*H_ini; 
   pvecback_integration[pba->index_bi_pp_eymh] = ppi_eymh*sqrt(H_ini);  
   pvecback_integration[pba->index_bi_sp_eymh] = spi_eymh*sqrt(H_ini); 
   pvecback_integration[pba->index_bi_v_eymh]  = Vi_eymh*H_ini;  
   pvecback_integration[pba->index_bi_s_eymh]  = Si_eymh*H_ini;       

    class_test(!isfinite(pvecback_integration[pba->index_bi_x_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_x_eymh]),
               pba->error_message,
               "initial x_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_x_eymh]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_y_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_y_eymh]),
               pba->error_message,
               "initial y_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_y_eymh]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_xi_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_xi_eymh]),
               pba->error_message,
               "initial xi_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_xi_eymh]); 
               
    class_test(!isfinite(pvecback_integration[pba->index_bi_pp_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_pp_eymh]),
               pba->error_message,
               "initial pp_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_pp_eymh]);      
              
    class_test(!isfinite(pvecback_integration[pba->index_bi_sp_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_sp_eymh]),
               pba->error_message,
               "initial sp_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_sp_eymh]);      
               
    class_test(!isfinite(pvecback_integration[pba->index_bi_z_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_z_eymh]),
               pba->error_message,
               "initial z_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_z_eymh]);      
               
    class_test(!isfinite(pvecback_integration[pba->index_bi_wp_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_wp_eymh]),
               pba->error_message,
               "initial wp_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_wp_eymh]);   
               
    class_test(!isfinite(pvecback_integration[pba->index_bi_v_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_v_eymh]),
               pba->error_message,
               "initial v_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_v_eymh]);                 
               
    class_test(!isfinite(pvecback_integration[pba->index_bi_s_eymh]) ||
               !isfinite(pvecback_integration[pba->index_bi_s_eymh]),
               pba->error_message,
               "initial s_eymh_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_s_eymh]);                    
  }      

     // - Fix initial value for anistropic vector field $

 if (pba->has_avf == _TRUE_) {      

   Zi_avf = 1.e-20;
   Vi_avf = pba->avf_parameters_2*1.e-26;
   Xi_avf = 0.001; 
   Si_avf = 1.e-18;


   H_ini = pba->H0*sqrt((pba->Omega0_g/pow(a,4.) + (pba->Omega0_b + pba->Omega0_cdm)/pow(a,3.))/(1. - Zi_avf*Zi_avf - Vi_avf*Vi_avf - Si_avf*Si_avf)); 

   pvecback_integration[pba->index_bi_z_avf] = Zi_avf*H_ini;
   pvecback_integration[pba->index_bi_v_avf] = Vi_avf*H_ini; 
   pvecback_integration[pba->index_bi_x_avf] = Xi_avf; 
   pvecback_integration[pba->index_bi_s_avf] = Si_avf*H_ini;       


    class_test(!isfinite(pvecback_integration[pba->index_bi_z_avf]) ||
               !isfinite(pvecback_integration[pba->index_bi_z_avf]),
               pba->error_message,
               "initial z_avf_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_z_avf]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_v_avf]) ||
               !isfinite(pvecback_integration[pba->index_bi_v_avf]),
               pba->error_message,
               "initial v_avf_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_v_avf]); 

    class_test(!isfinite(pvecback_integration[pba->index_bi_x_avf]) ||
               !isfinite(pvecback_integration[pba->index_bi_x_avf]),
               pba->error_message,
               "initial x_avf_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_x_avf]);     

    class_test(!isfinite(pvecback_integration[pba->index_bi_s_avf]) ||
               !isfinite(pvecback_integration[pba->index_bi_s_avf]),
               pba->error_message,
               "initial s_avf_ini = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_s_avf]);                      
  }
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_anq",pba->has_anq);
  class_store_columntitle(titles,"(.)p_anq",pba->has_anq);
  class_store_columntitle(titles,"x_anq",pba->has_anq);
  class_store_columntitle(titles,"y_anq",pba->has_anq);
  class_store_columntitle(titles,"z_anq",pba->has_anq);
  class_store_columntitle(titles,"sigma_anq",pba->has_anq);  

  class_store_columntitle(titles,"(.)rho_sde",pba->has_sde);
  class_store_columntitle(titles,"(.)p_sde",pba->has_sde);
  class_store_columntitle(titles,"f1_sde",pba->has_sde);
  class_store_columntitle(titles,"f2_sde",pba->has_sde);     
  class_store_columntitle(titles,"sigma_sde",pba->has_sde);  

  class_store_columntitle(titles,"(.)rho_a2f",pba->has_a2f);
  class_store_columntitle(titles,"(.)p_a2f",pba->has_a2f);
  class_store_columntitle(titles,"x1_a2f",pba->has_a2f);
  class_store_columntitle(titles,"x2_a2f",pba->has_a2f); 
  class_store_columntitle(titles,"OmB_a2f",pba->has_a2f);        
  class_store_columntitle(titles,"sigma_a2f",pba->has_a2f); 

  class_store_columntitle(titles,"(.)rho_eymh",pba->has_eymh);
  class_store_columntitle(titles,"(.)p_eymh",pba->has_eymh);
  class_store_columntitle(titles,"x_eymh",pba->has_eymh);
  class_store_columntitle(titles,"y_eymh",pba->has_eymh);  
  class_store_columntitle(titles,"xi_eymh",pba->has_eymh);
  class_store_columntitle(titles,"pp_eymh",pba->has_eymh);
  class_store_columntitle(titles,"s_eymh",pba->has_eymh);
  class_store_columntitle(titles,"z_eymh",pba->has_eymh); 
  class_store_columntitle(titles,"w_eymh",pba->has_eymh);
  class_store_columntitle(titles,"v_eymh",pba->has_eymh);     
  class_store_columntitle(titles,"sigma_eymh",pba->has_eymh); 

  class_store_columntitle(titles,"(.)rho_avf",pba->has_avf);
  class_store_columntitle(titles,"(.)p_avf",pba->has_avf);
  class_store_columntitle(titles,"z_avf",pba->has_avf);
  class_store_columntitle(titles,"v_avf",pba->has_avf);
  class_store_columntitle(titles,"x_avf",pba->has_avf);
  class_store_columntitle(titles,"sigma_avf",pba->has_avf); 
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_anq],pba->has_anq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_anq],pba->has_anq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_x_anq],pba->has_anq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_y_anq],pba->has_anq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_z_anq],pba->has_anq,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_s_anq],pba->has_anq,storeidx);  

    class_store_double(dataptr,pvecback[pba->index_bg_rho_sde],pba->has_sde,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_sde],pba->has_sde,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_f1_sde],pba->has_sde,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_f2_sde],pba->has_sde,storeidx);     
    class_store_double(dataptr,pvecback[pba->index_bg_s_sde],pba->has_sde,storeidx); 

    class_store_double(dataptr,pvecback[pba->index_bg_rho_a2f],pba->has_a2f,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_a2f],pba->has_a2f,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_x1_a2f],pba->has_a2f,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_x2_a2f],pba->has_a2f,storeidx);  
    class_store_double(dataptr,pvecback[pba->index_bg_OmB_a2f],pba->has_a2f,storeidx);           
    class_store_double(dataptr,pvecback[pba->index_bg_s_a2f],pba->has_a2f,storeidx);  

    class_store_double(dataptr,pvecback[pba->index_bg_rho_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_x_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_y_eymh],pba->has_eymh,storeidx);   
    class_store_double(dataptr,pvecback[pba->index_bg_xi_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_pp_eymh],pba->has_eymh,storeidx);  
    class_store_double(dataptr,pvecback[pba->index_bg_sp_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_z_eymh],pba->has_eymh,storeidx);   
    class_store_double(dataptr,pvecback[pba->index_bg_wp_eymh],pba->has_eymh,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_v_eymh],pba->has_eymh,storeidx);       
    class_store_double(dataptr,pvecback[pba->index_bg_s_eymh],pba->has_eymh,storeidx);  

    class_store_double(dataptr,pvecback[pba->index_bg_rho_avf],pba->has_avf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_avf],pba->has_avf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_z_avf],pba->has_avf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_v_avf],pba->has_avf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_x_avf],pba->has_avf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_s_avf],pba->has_avf,storeidx); 
...
int background_derivs(
  ...
  double x_anq, y_anq, z_anq, s_anq;
  double l_anq, mu_anq;

  double f1_sde, f2_sde, s_sde; 
  double m_sde, n_sde;

  double x1_a2f, x2_a2f, OmB_a2f, s_a2f;  
  double l_a2f, m_a2f;

  double x_eymh, y_eymh, xi_eymh, pp_eymh, sp_eymh, z_eymh, wp_eymh, v_eymh, s_eymh;
  double ap_eymh, Wpi_eymh;    

  double z_avf, v_avf, x_avf, s_avf;
  double l_avf;
  ...
  /** - Compute the anisotropic quintaessence equation of motions. \f$ dx_{anq}/dlna = x_{anq}' \f$  */
  if (pba->has_anq == _TRUE_) { 

    l_anq   = pba->anq_parameters_1;             // \lambda
    mu_anq  = pba->anq_parameters_2;              // \mu

    x_anq = y[pba->index_bi_x_anq];
    y_anq = y[pba->index_bi_y_anq];
    z_anq = y[pba->index_bi_z_anq];    
    s_anq = y[pba->index_bi_s_anq];    

    dy[pba->index_bi_x_anq] = -sqrt(6.)*(sqrt(6.)*x_anq - l_anq*(x_anq*x_anq + y_anq*y_anq)/H + 2.*mu_anq*z_anq*z_anq/H)/2.;   
    dy[pba->index_bi_y_anq] = -sqrt(6.)*l_anq*x_anq*y_anq/H/2.;   
    dy[pba->index_bi_z_anq] = z_anq*(sqrt(6.)*mu_anq*x_anq/H -2.*s_anq/H -2.);   
    dy[pba->index_bi_s_anq] = -3.*s_anq + 2.*z_anq*z_anq/H;       
  }

  /**   Solid dark energy sde */
    /** - Compute the solid equations of motions. \f$ df_{1}/dlna = f_{1}' \f$  */
  if (pba->has_sde == _TRUE_) { 

    n_sde  = pba->sde_parameters_1;   //E^{n}         
    m_sde  = pba->sde_parameters_2;  //E^{m}
    
    f1_sde = y[pba->index_bi_f1_sde];
    f2_sde = y[pba->index_bi_f2_sde];
    s_sde  = y[pba->index_bi_s_sde];    

    dy[pba->index_bi_f1_sde] = -n_sde*f1_sde*(1.-2.*s_sde/H);   
    dy[pba->index_bi_f2_sde] = -m_sde*f2_sde*(1.+s_sde/H);
    dy[pba->index_bi_s_sde]  = -3.*s_sde -(2./H)*(n_sde*f1_sde*f1_sde - m_sde*f2_sde*f2_sde);       
  }

  /**   Anisotropic 2-form (a2f)  */
  /** - Compute the anisotropic 2-form equation of motions. \f$ dx1_{a2f}/dlna = x1_{a2f}' \f$  */  
  if (pba->has_a2f == _TRUE_) { 

    l_a2f  = pba->a2f_parameters_1;         // \lambda
    m_a2f  = pba->a2f_parameters_2;         // \mu
    
    x1_a2f  = y[pba->index_bi_x1_a2f];
    x2_a2f  = y[pba->index_bi_x2_a2f];
    OmB_a2f = y[pba->index_bi_OmB_a2f];    
    s_a2f   = y[pba->index_bi_s_a2f];    

    dy[pba->index_bi_x1_a2f]  = -3.*x1_a2f + sqrt(6.)*((l_a2f*x2_a2f*x2_a2f)/H - (m_a2f*OmB_a2f)/H )/2.;       
    dy[pba->index_bi_x2_a2f]  = -sqrt(6.)/2*(l_a2f*x1_a2f*x2_a2f)/H;
    dy[pba->index_bi_OmB_a2f] = -2.*OmB_a2f + 4.*OmB_a2f*s_a2f/H + sqrt(6.)*(m_a2f*OmB_a2f*x1_a2f)/H;      
    dy[pba->index_bi_s_a2f]   = -3.*s_a2f - 2.*OmB_a2f/H;
  }

  /**   Einstein Yang-Mills Higgs  */
  /** - Compute the Einstein Yang-Mills Higgs equation of motions. \f$ dx_{eymh}/dlna = x_{eymh}' \f$  */  
  if (pba->has_eymh == _TRUE_) { 

    ap_eymh   = pba->eymh_parameters_1;      
    
    x_eymh  = y[pba->index_bi_x_eymh];
    y_eymh  = y[pba->index_bi_y_eymh];
    xi_eymh  = y[pba->index_bi_xi_eymh];
    pp_eymh = y[pba->index_bi_pp_eymh];
    sp_eymh = y[pba->index_bi_sp_eymh];
    z_eymh  = y[pba->index_bi_z_eymh];
    wp_eymh = y[pba->index_bi_wp_eymh];
    v_eymh  = y[pba->index_bi_v_eymh];
    s_eymh  = y[pba->index_bi_s_eymh];    

    dy[pba->index_bi_x_eymh]  = -2.*x_eymh*(1.+s_eymh/H)-(2.*pp_eymh*sp_eymh*sp_eymh*sp_eymh*xi_eymh)/H;       
    dy[pba->index_bi_y_eymh]  = y_eymh*(-2.+s_eymh/H)-(xi_eymh/H)*(pp_eymh*pp_eymh*sp_eymh*sp_eymh + sp_eymh*sp_eymh*sp_eymh*sp_eymh + wp_eymh*wp_eymh);
    dy[pba->index_bi_pp_eymh] = pp_eymh*(-1.+(2.*s_eymh)/H)+(xi_eymh*sp_eymh*x_eymh)/H;     
    dy[pba->index_bi_sp_eymh] = sp_eymh*(-1. + (y_eymh*xi_eymh)/H - s_eymh/H);    
    dy[pba->index_bi_v_eymh]  = ap_eymh*(wp_eymh*z_eymh*xi_eymh)/H;     
    dy[pba->index_bi_wp_eymh] = wp_eymh*(-1. + (xi_eymh*y_eymh)/H - s_eymh/H)+ sqrt(2.)*xi_eymh*(z_eymh*sp_eymh*sp_eymh)/H;      
    dy[pba->index_bi_z_eymh]  = -3.*z_eymh - (wp_eymh*xi_eymh)*(sqrt(2.)*sp_eymh*sp_eymh + ap_eymh*v_eymh)/H;      
    dy[pba->index_bi_xi_eymh] = xi_eymh*(1. - y_eymh*xi_eymh/H + s_eymh/H);     
    dy[pba->index_bi_s_eymh]  = -3.*s_eymh + (sp_eymh*sp_eymh)*(sp_eymh*sp_eymh - pp_eymh*pp_eymh)/H + wp_eymh*wp_eymh/H - y_eymh*y_eymh/H + x_eymh*x_eymh/H; 
  }      

  /**   Anisotropic dark energy  */
  /** - Compute the anisotropic vector field equation of motions. \f$ dx_{avf}/dlna = x_{avf}' \f$  */
  if (pba->has_avf == _TRUE_) { 

    l_avf   = pba->avf_parameters_1;             // \lambda

    z_avf = y[pba->index_bi_z_avf];    
    v_avf = y[pba->index_bi_v_avf];
    x_avf = y[pba->index_bi_x_avf];    
    s_avf = y[pba->index_bi_s_avf];    

    dy[pba->index_bi_z_avf] = -z_avf*(1. + s_avf/H) - sqrt(3.)*l_avf*x_avf*v_avf*v_avf/H/sqrt(2.);
    dy[pba->index_bi_v_avf] = sqrt(6.)*l_avf*x_avf*z_avf*v_avf/H/2. + l_avf*x_avf*x_avf*v_avf*(s_avf/H - 1./2.);
    dy[pba->index_bi_x_avf] =  sqrt(6.)*z_avf/H +x_avf*(2.*s_avf/H -1.);
    dy[pba->index_bi_s_avf] = -3.*s_avf + 2.*z_avf*z_avf/H - l_avf*x_avf*x_avf*v_avf*v_avf/H;       
  }  
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if ((pba->has_lambda == _TRUE_) || (pba->has_fld == _TRUE_) || (pba->has_scf == _TRUE_) || (pba->has_anq == _TRUE_) || (pba->has_sde == _TRUE_)  || (pba->has_a2f == _TRUE_) || (pba->has_eymh == _TRUE_) || (pba->has_avf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
    ...
    if (pba->has_anq == _TRUE_) {
      class_print_species("Anisotropic quintaessence",lambda);
      budget_other+=pba->Omega0_lambda;
    } 
    if (pba->has_sde == _TRUE_) {
      class_print_species("Solid ",lambda);
      budget_other+=pba->Omega0_lambda;
    } 
    if (pba->has_a2f == _TRUE_) {
      class_print_species("Anisotropic 2-form ",lambda);
      budget_other+=pba->Omega0_lambda;
    }
    if (pba->has_eymh == _TRUE_) {
      class_print_species("Eintein Yang-Mills Higgd ",lambda);
      budget_other+=pba->Omega0_lambda;
    }                   
    if (pba->has_avf == _TRUE_) {
      class_print_species("Anisotropic vector field",lambda);
      budget_other+=pba->Omega0_lambda;
    }    
